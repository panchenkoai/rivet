...
| **Campaign / waves** | When `rivet plan` is run on a multi-export config, the artifact embeds a `CampaignRecommendation` — exports sorted by an advisory priority score, grouped into execution `waves`, plus `source_group` warnings when several heavy exports share a replica (e.g. `isolate_on_source: true`). The priority score and grouping are advisory ([ADR-0006](adr/0006-source-aware-prioritization.md)), but Rivet can execute the waves itself: `rivet apply <config>.yaml` runs the exports wave-by-wave in ascending `wave:` order (with `--parallel-export-processes`, `--resume`, or `--pool`/`--split` scheduling modes); an external scheduler (Airflow / cron / GH Actions) is optional. Demoed in [plan-campaign.gif](gifs/plan-campaign.gif). |
