...
> **Отложено (документировано):** V19 rustls-webpki 0.101.7 — tiberius 0.12 пинит rustls 0.21; upgrade-path = bump tiberius или gate MSSQL за non-default feature (продуктовое решение). V22 per-value ceiling аллоцирует один раз до guard'а — структурный (нужен source-side SUBSTRING cap); guard включён по умолчанию (Some(256)). **Follow-up:** dev/CI seed-скрипты (Makefile, .github/workflows, dev/*.sh) надо обновить на `--features dev-seed` + `RIVET_SEED_I_KNOW=1` (dev-only, не shipped).
>
