...
| **Completeness** | Full cycle: extract → typed parquet + manifest → `rivet load` into BigQuery / Snowflake; with `mode: cdc` in the config's `load:` block, the load maintains a current-state dedup view. | Full cycle: extract → MERGE → dedup → metadata. | — |
