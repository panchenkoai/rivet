...
| `oneshot_budget_mb` | `integer` |  | Cap on the RAM one-shot (single-PUT) upload buffers may hold, in MB (default 64). A one-shot PUT buffers the whole part so the store records a content checksum (`Content-MD5`, checked by `validate`); a part that does not fit the remaining budget streams instead (memory-bounded, size-only verification). `0` streams every part. The cap is process-wide per value: every destination with the same budget — including each table of a CDC export — draws from one pool of that size, so it never multiplies. |
