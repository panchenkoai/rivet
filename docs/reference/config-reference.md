...
| `content_hash` | [ContentHashConfig](#contenthashconfig) |  | Append the extraction-time canonical content hash (`__content_hash`) to every part this export writes — see [`ContentHashConfig`]. Unlike `meta_columns`, this DOES apply to CDC (both the snapshot leg and the change stream): the hash is a data-carrying column the warehouse keeps, not run metadata. |
