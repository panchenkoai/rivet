...
- **Dense-chunking completeness on a concurrently-written table.** `chunk_dense: true` pages by `ROW_NUMBER() OVER (ORDER BY chunk_column)`, recomputed in an independent query — and an independent read snapshot — per chunk. Any row another session inserts or deletes during the run shifts every later ordinal, so rows move across window boundaries between chunk queries: a committed DELETE below a boundary silently **skips** rows that existed for the whole run, a committed INSERT **duplicates** them. This does **not** depend on ties — it happens on a unique primary key. Tied peer groups add a second, smaller risk (two queries ordering the tied band differently). Against a **static** table this does not occur on any tested engine (PG 16 / MySQL 8 / SQL Server 2022 — verified by `tests/live/live_chunked_dense.rs`). rivet warns at run start whenever `chunk_dense` is used. On a live-writing table use **keyset** (`chunk_by_key`) on a unique key, which seeks by key value and never re-numbers.
