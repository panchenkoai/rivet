...
**Amendment (2026-09-17) — the spec is keyed by RUN, not only by name.** The by-name row is last-writer-wins, and a state DB is shared by design (ADR on shared deployments): two configs whose exports share a name (`users` from PostgreSQL, `users` from MongoDB) retyped each other's table between a run and its load — measured as a `_id` key on a PostgreSQL table and another engine's column types in the DDL. `rivet run` now also records the spec under its run id (`export_load_spec_run`, schema v28, `(export_name, unit, run_id)`), and `rivet load` pins each table's plan to the newest run under its own prefix that carries a row (`pin_plan_to_its_run`), rebuilding DDL and `pk: auto` from it. A run's row cannot see the key `rivet init` recorded, so it borrows only the `init`-origin by-name key, never another run's. Runs older than v28, or a prefix the load cannot list, keep the by-name spec and say so; a column the pinned spec does not know is a refusal for that table, with the fix named (`rivet run -e <export>` once). The `columns:` a load is typed from are therefore the newest run's recorded columns, not the live source.
...
**Default for an append table (incremental / CDC):** the change log follows the table it grows from. An incremental export's first run re-reads the whole table (ADR-0033 MT1) and lands as a plain `<table>` ONLY when that name is still free, exactly like a full load; the first delta renames that table to `<table>__changes` (`ALTER TABLE … RENAME TO`, then `ADD COLUMN __op, __pos, __seq`), so the log keeps whatever partitioning and clustering the table had — rivet's own `cluster_by`, or a shape the operator gave it by hand — and the current-state view takes the old name. The load tells a whole-table run from a delta by the manifest's `extraction.cursor_low` (absent when the run had no cursor to resume from). An incremental load NEVER overwrites a `<table>` that already exists: the rename happens first and the whole pass is appended to the log, so the log also keeps rows the source has since deleted. The overlap between the kept baseline and the re-read pass is at-least-once and the current-state view dedups it by key — that duplication is the deliberate price of never replacing an existing table. No copy and no query: the earlier `CREATE TABLE … AS SELECT` adoption was billed and, after a full → incremental switch, duplicated every row the first run had re-read. The same rename applies when a keyset export continues as incremental on the same key and when a CDC stream starts over a full-load table without `initial: snapshot`; a CDC load carrying a snapshot over such a table is refused (keep one baseline). On BigQuery the renamed log drops `require_partition_filter` (the view reads the whole log) and the expiry of ingestion-time partitions (expiring load dates would drop unchanged rows from the view); a `LOAD DATA INTO` declaring the meta columns first appends to a log where they are last (verified 2026-09-11). Counts come from `tables.get` metadata, since `COUNT(*)` on a table requiring a partition filter is refused. Snowflake uses the same rename script; not live-run. No main table → no partition.
...
### D7 — `layout` and `deleted_flag`: where the current state lives is a WRITTEN choice (2026-09-20)

- **The two layouts.** `log_view`: `<table>__changes` is an append-only changelog and `<table>` a dedup view over it (latest per `pk`) — the shape every incremental and capture-only CDC export has had. `base_buffer`: `<table>` is a PHYSICAL base (source columns, plus `__is_deleted` carried as data), `<table>__changes` a per-cycle buffer, and `rivet compact` MERGEs the buffer into the base (latest per key; a delete flags the row and keeps its last values, a re-insert un-flags) and drops it. Measured before the key existed: a view over a 317M-row log re-ranked the whole log on every read with no partition pruning (a full scan against 19 MiB for the same question on a pruned base).
- **Written, not inferred from the mode.** `load.layout: log_view | base_buffer` at the section, per export, and per captured table (`load.tables.<t>`), layered like every other key (D2). Absent keeps what shipped — a CDC stream with a `backfill:` is base+buffer, everything else log+view — so no existing deployment changes shape. `full` ignores it (an overwrite has no accumulated state). The extract asks the SAME resolver as the load (`resolved_layout`): a base's rows carry `__is_deleted` as data only the writer can put in the file, and a leg that derived the layout from `backfill:` presence alone wrote a snapshot without it (`LOAD DATA` fills an absent column with NULL, never a default).
- **`deleted_flag`** decides whether the base carries `__is_deleted`; absent derives from the mode (a stream expresses deletes, a query cannot). Without the flag a delete DELETEs the base row — there is nothing to flag.
- **BigQuery only.** `rivet compact` is BigQuery-only in this release, so a written `base_buffer` is refused at config load for another target and the `backfill:` derivation yields log+view there — a base no compaction can merge is a snapshot frozen at the backfill with a buffer that grows for ever.
- **The compaction's contract.** One scripted job per table: the distinct partitions of every row it touches — the buffer's AND the base's rows of the buffer's keys, since a key's OLD value sits in the base under a day the buffer never names — bound the base side (plus its NULL partition); winners are taken in chunks of 4,000 partitions; a load-date key merges unbounded; the buffer is dropped inside the script, so a crash between MERGE and DROP re-merges idempotently. A whole-table pass over a base whose buffer still holds rows is refused (the older rows would win the next merge); a base created through the batched load's `CLONE` is a table; a `9999-12-31` value has no successor and leaves the upper bound open.
- **Not decided here:** an owner config's whole-table pass after `state reset` replaces the base in place (D5's append-to-log rule was written for `log_view`); whether the buffer should accept a `partition:` of its own (today it takes none and is read whole).

---

## Rollout

1. **Metabase load spec + source-free load** (D1). RED first: a `rivet load` whose `source.url` points at a closed port must succeed after a `rivet run`, and fail loudly with no metabase row. **Done (2026-09-11):** `export_load_spec` (v27), recorded after every successful run in the `run_export_job` wrapper and on `apply`; live on MySQL, PostgreSQL, SQL Server and MongoDB, end to end on BigQuery with the source on a closed port. `rivet init` recording the key moves to step 2, where it is first used. **Per-run pin (2026-09-17):** v28 `export_load_spec_run`, RED first on two `users` exports over one PostgreSQL state DB (the load exited 0 with a `["_id","w"]` catalog) — `a_load_is_typed_from_its_own_run_not_from_the_last_writer_of_its_name`; the state-parity gate cell now fails when no spec row is recorded.
...
4. **Spec drift** (D5) for `__changes`: a changed `partition` refused naming the rebuild, `rivet load --rebuild-changelog`, a changed `cluster_by` applied to the log's metadata. **Done (2026-09-12):** drift is judged against what the config WRITES — a `partition:` block, a `cluster_by` list or `none` — so an adopted log keeps the shape its table had under the defaults (D2), and follows the config once something is declared. A declared partition that differs is refused naming both keys and the bytes the rebuild reads (`tables.get numBytes`); `--rebuild-changelog` copies the log with `CREATE TABLE … AS SELECT *` in the declared shape, checks the row count, renames the old log aside, renames the copy in, drops the old one — and a load that finds `__changes__old` / `__changes__rebuild` from an interrupted swap stops before touching anything. A declared clustering that differs is patched in place (`tables.patch clustering`, verified: set and clear, 2026-09-12); a changed expiry is altered in place as on the full table. Live on BigQuery: re-cluster in place, refusal, rebuild. The rebuild's copy carries the log's description, labels, friendly name, table expiry and KMS key (a `CREATE TABLE … AS SELECT` carries rows only); a log whose columns carry policy tags, or that has row access policies, is refused — no copy carries those — and rebuilt by hand. The same "follows what is written" rule applies to the full table: an unwritten `cluster_by` (`auto`) follows the clustering the existing table has, so a table an earlier release created unclustered is overwritten as it is, not refused after the upgrade.
