...
| G5 | ✅ | P0 | E2E matrix in CI — `ci.yml::e2e` runs both `python3 -m dev.pytools.e2e` **and** `cargo test --release -- --ignored` (**285** ignored live tests across **51** `tests/live_*.rs` binaries, including **Trust golden** parity on Postgres + MySQL in `live_type_golden.rs`). See [docs/reference/testing.md](docs/reference/testing.md). |
...
| I2 | ✅ | P1 | `cargo bench` + `python3 -m dev.pytools.dev_scripts bench` save/compare; column_scan + shape_tracking groups |
...
4. ✅ **I2** — `cargo bench` + `python3 -m dev.pytools.dev_scripts bench` save/compare harness; column_scan + shape_tracking groups.
