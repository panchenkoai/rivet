...
### H2. In-flight load skip-set cell never runs on the release gate — pre-existing — **FIXED** (fix/open-issues-r3: the cell forces SQLite via an empty RIVET_STATE_URL)
...
### H7. The in-flight cell grades a COPY of the active-run predicate — pre-existing
