...
#
# Usage: bash dev/scripts/test_permissions.sh
...
START=$(date +%s)
OUTPUT=$(RUST_LOG=error $RIVET run --config dev/fixtures/test_pg_noaccess.yaml 2>&1 || true)
END=$(date +%s)
ELAPSED=$((END - START))
...
OUTPUT=$(RUST_LOG=error $RIVET run --config dev/fixtures/test_pg_partial.yaml 2>&1 || true)
...
START=$(date +%s)
OUTPUT=$(RUST_LOG=error $RIVET run --config dev/fixtures/test_mysql_noaccess.yaml 2>&1 || true)
END=$(date +%s)
ELAPSED=$((END - START))
...
OUTPUT=$(RUST_LOG=error $RIVET run --config dev/fixtures/test_mysql_partial.yaml 2>&1 || true)
...
START=$(date +%s)
OUTPUT=$(RUST_LOG=error $RIVET run --config dev/fixtures/test_pg_wrongpass.yaml 2>&1 || true)
END=$(date +%s)
ELAPSED=$((END - START))
