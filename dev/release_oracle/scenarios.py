...
# ── config generation ────────────────────────────────────────────────────────
_INIT_CFG_LOCK = threading.Lock()


def _init_cfg(engine: str, url: str, schema: str) -> Path | None:
    """`rivet init` a config ONCE per engine FAMILY, reused by every version.

    One file per (engine, schema): the first version that asks generates it, the
    rest are handed the same path. That is coverage, not just fewer init calls —
    the config born on the OLDEST version is what runs against the NEWEST, so the
    run tests config PORTABILITY across a server upgrade, which nothing here did
    before. `url_env: ORACLE_URL` is what makes it safe: the generated file
    carries no host and no port, only the variable name.

    Measured 2026-09-20: init emits BYTE-IDENTICAL configs across postgres 12-17,
    and across mongo 4.4/8 bar collection order — so generating one PER VERSION
    bought no present coverage, only a guard against future divergence.

    Reuse is also what fixes the original defect rather than working around it:
    `rivet init` REFUSES to overwrite, so a shared name used to let the first
    version of each family create the file and every LATER version fail init,
    silently skipping its verdict cell — 4 of 15 cells graded, 11 skipped
    (postgres 13-18, mongo 5-8, mysql 8.4), measured on two full-matrix runs.
    Asking init only when the file is absent means that refusal never fires.
    ...
    # Serialised because versions of one family can run CONCURRENTLY: without the
    # lock several would see the file absent at the same instant, all call init on
    # the same path, and all but one hit the no-overwrite refusal — the original
    # defect back in a new costume. Check-and-generate has to be one step.
    with _INIT_CFG_LOCK:
        # Generated by an earlier version of this family: hand it over rather than
        # re-running init, which would hit that same refusal.
        if out.is_file():
            return out
        ...
        if schema:
            argv += ["--schema", schema]
        argv += ["-o", str(out)]
        ok = rivet(*argv, env={"ORACLE_URL": url}, timeout=NO_TIMEOUT).ok
...
def verify_inflight_run_stays_loadable(led: Ledger) -> None:
    ...
    show = run([str(rivet_bin()), "state", "show", "-c", str(cfgf)],
               # An empty RIVET_STATE_URL keeps this cell on the SQLite file beside the
               # config on every pass, Postgres ones included.
    ...
    if not db.exists():
        _failed(led, "load", "inflight", "skipset", "-",
                f"in-flight load skip-set: `rivet state show` with no RIVET_STATE_URL left no "
                f"SQLite state at {db}",
                "no sqlite state")
        return
