...
run pg_plan_param_used  "plan PG with --param matching query" -- "$R" plan -c "$PGC" -e pa_audit --param max_id=20 --format json
run pg_plan_param_unused "plan PG with --param unused"    -- "$R" plan -c "$PGC" -e pa_audit --param unused=value --format json
