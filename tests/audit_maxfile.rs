...
fn audit_maxfilesize_parquet_warns_when_inert() {
    ...
    // content_items is pre-seeded; id <= 6000 is ~6000 wide rows (text + jsonb).
    // Guard the precondition so a thin DB fails loudly here rather than silently
    // weakening the assertion.
    let count: i64 = {
        use postgres::{Client, NoTls};
        let mut c = Client::connect(POSTGRES_URL, NoTls).expect("connect postgres");
        c.query_one("SELECT COUNT(*) FROM content_items WHERE id <= 6000", &[])
            .expect("count content_items")
            .get(0)
    };
    assert!(
        count >= 1000,
        "precondition: content_items needs >= 1000 rows with id <= 6000 (has {count}). \
...
exports:
  - name: {export}
    query: "SELECT id, title, body, raw_html, metadata FROM content_items WHERE id <= 6000 ORDER BY id"
    mode: full
    format: parquet
    compression: zstd
    max_file_size: 64KB
