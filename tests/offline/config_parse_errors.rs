...
/// `cdc.snapshot` tunes the `initial: snapshot` LEG. Every refusal below is a
/// combination that would otherwise be accepted and then do nothing visible —
/// the accept-but-silently-ignore shape, which on a snapshot means "reads for
/// hours on one connection while the config says four workers".
///
/// Graded in BOTH directions, because a gate that refuses everything is the bug
/// that replaces the one it fixed: the last case asserts a valid block parses and
/// keeps its values, including the conservative `parallel: 1` default.
#[test]
fn cdc_snapshot_block_is_refused_when_inert_or_unrunnable() {
    let pg = |cdc: &str| {
        format!(
    ...
    // No `initial:` ⇒ no leg to configure. The block would sit there inert.
    let err = parse_err(&pg("{ snapshot: { chunk_by_key: id } }"));
    assert!(
        err.contains("initial"),
        "the refusal must point at the missing `cdc.initial`; got: {err}"
    );

    // An empty key cannot page anything, and the planner's own refusal would only
    // arrive after the anchor was taken.
    let err = parse_err(&pg(
        "{ initial: snapshot, snapshot: { chunk_by_key: \"\" } }",
    ));
    assert!(
        err.contains("chunk_by_key"),
        "the refusal must name the empty key; got: {err}"
    );

    // Zero workers / zero rows per page: arithmetic that reads nothing.
    let err = parse_err(&pg(
        "{ initial: snapshot, snapshot: { chunk_by_key: id, parallel: 0 } }",
    ));
    assert!(
        err.contains("parallel"),
        "the refusal must name `parallel`; got: {err}"
    );
    let err = parse_err(&pg(
        "{ initial: snapshot, snapshot: { chunk_by_key: id, chunk_size: 0 } }",
    ));
    assert!(
        err.contains("chunk_size"),
        "the refusal must name `chunk_size`; got: {err}"
    );

    // MongoDB's leg reads documents, not a keyset over a SQL key.
    let err = parse_err(
        "\nsource:\n  type: mongo\n  url: \"mongodb://localhost:27017/db\"\n\
         exports:\n  - name: t\n    table: t\n    mode: cdc\n    format: parquet\n\
         \x20   cdc: { initial: snapshot, checkpoint: /tmp/ck, snapshot: { chunk_by_key: _id } }\n\
         \x20   destination: { type: local, path: \"/tmp/x\" }\n",
    );
    assert!(
        err.contains("MongoDB") || err.contains("mongo"),
        "the refusal must name the engine that cannot run it; got: {err}"
    );

    // The valid shape parses, and an omitted `parallel` stays at one worker — the
    // default that keeps source pressure unchanged for anyone who just adds a key.
    let cfg = Config::from_yaml(&pg("{ initial: snapshot, snapshot: { chunk_by_key: id } }"))
        .expect("a well-formed snapshot block must parse");
    let snap = cfg.exports[0]
        .cdc
        .as_ref()
        .and_then(|c| c.snapshot.as_ref())
        .expect("the block survives parsing");
    assert_eq!(snap.chunk_by_key, "id");
    assert_eq!(
        snap.parallel, 1,
        "omitted parallel must not fan workers out"
    );
    assert_eq!(snap.chunk_size, None);
}
