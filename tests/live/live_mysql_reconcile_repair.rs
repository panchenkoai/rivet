//! Live E2E tests for `rivet reconcile` and `rivet repair` against MySQL —
//! MySQL twin of `tests/live_reconcile_repair.rs`.
//!
...
//! file mirrors the RR1-RR6 matrix against MySQL to ensure the
//! chunked-aware partition-counting and JSON contract both hold there.
//!
//! ## Test symmetry with `live_reconcile_repair.rs`
//!
//! | PG test                                                          | MySQL twin (this file)                                                 |
//! |------------------------------------------------------------------|------------------------------------------------------------------------|
//! | `reconcile_all_match_exits_zero_with_pretty_output`              | `mysql_reconcile_all_match_exits_zero_with_pretty_output`              |
//! | `reconcile_format_json_emits_valid_json_report`                  | `mysql_reconcile_format_json_emits_valid_json_report`                  |
//! | `reconcile_format_json_output_flag_writes_report_to_file`        | `mysql_reconcile_format_json_output_flag_writes_report_to_file`        |
//! | `repair_report_flag_uses_precomputed_reconcile_json`             | `mysql_repair_report_flag_uses_precomputed_reconcile_json`             |
//! | `repair_dryrun_exits_zero_when_nothing_to_repair`                | `mysql_repair_dryrun_exits_zero_when_nothing_to_repair`                |
//! | `repair_execute_flag_exits_zero_when_nothing_to_repair`          | `mysql_repair_execute_flag_exits_zero_when_nothing_to_repair`          |
//! | `repair_format_json_emits_valid_json_plan`                       | `mysql_repair_format_json_emits_valid_json_plan`                       |

use crate::common::*;

/// Seed a row_count-row MySQL table, write a chunked config with checkpoint,
/// run the export, then return the config path (plus the table RAII guard).
fn seed_and_run_chunked(row_count: i64, chunk_size: u32) -> (MysqlTable, Rig) {
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_reconcile_format_json_emits_valid_json_report() {
    require_alive(LiveService::Mysql);
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_reconcile_format_json_output_flag_writes_report_to_file() {
    require_alive(LiveService::Mysql);
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_repair_report_flag_uses_precomputed_reconcile_json() {
    require_alive(LiveService::Mysql);
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_repair_dryrun_exits_zero_when_nothing_to_repair() {
    require_alive(LiveService::Mysql);
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_repair_execute_flag_exits_zero_when_nothing_to_repair() {
    require_alive(LiveService::Mysql);
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_repair_format_json_emits_valid_json_plan() {
    require_alive(LiveService::Mysql);
