...
/// Finding 2 (window persistence): a split resume must NOT re-sample the partition
/// against a source that GREW between the crash and the resume.
///
/// `sample_key_boundaries` picks boundaries at ROW-OFFSET percentiles, so they
/// SHIFT when rows are inserted. Before the fix, resume re-sampled the grown
/// source, got shifted boundaries, then skipped the "completed" units BY ORDINAL
/// NAME — so the crashed unit re-ran over a window that no longer lines up with
/// what its neighbours already covered, silently dropping a band of the ORIGINAL
/// key range. Inserting between the runs is not exotic: it is the normal state of
/// any live table.
///
/// The fix persists each unit's window in its manifest and reconstructs the EXACT
/// original partition on resume (`reconstruct_units_from_prefix`), ignoring the
/// grown source.
///
/// RED mechanism (verified by disabling `reconstruct_units_from_prefix`): with a
/// pre-crash checkpoint present, the shifted re-sampled window changes the crashed
/// unit's query, so the chunk-plan-fingerprint guard REFUSES to resume ("chunk
/// plan fingerprint mismatch") and the whole run FAILS — the fingerprint guard is
/// the backstop that turns what would otherwise be a silent gap into a loud abort.
/// The `resumed.status.success()` assert is what goes RED there. The `missing`
/// id-set check is belt-and-suspenders for the case the backstop is ABSENT (a unit
/// crashed before its first chunk committed → no fingerprint to mismatch → a fresh
/// re-run over the shifted window would silently gap). With the fix the resume
/// succeeds AND every original id is present.
///
/// Oracle: every ORIGINAL id must be DECLARED by the manifest after resume — read
/// with the orphan-immune `dir_manifest_copy_id_set`, since the crash leaves
/// pre-shift orphans on disk that a raw-parquet read would count (false-clean).
/// Whether the freshly-inserted ids also land depends on which unit held the open
/// (`hi=None`) window, so this asserts only the finding-2 guarantee: the original
/// snapshot survives the resume, gap-free.
#[test]
#[ignore = "live: requires docker compose up -d postgres"]
fn pool_split_resume_after_source_growth_keeps_the_original_partition_gap_free() {
    require_alive(LiveService::Postgres);
    const N: i64 = 300_000;
    const GROWTH: i64 = 150_000; // +50% between crash and resume → boundaries shift hard
    let table = unique_name("pr_grow");
    let mut c = pg_connect();
    c.batch_execute(&format!(
        "DROP TABLE IF EXISTS {table};
         CREATE TABLE {table} AS SELECT g AS id, md5(g::text) AS a \
           FROM generate_series(1, {N}) g;
         ALTER TABLE {table} ADD PRIMARY KEY (id);"
    ))
    .unwrap();

    // A small sibling export makes the giant the clear long pole so `advise_split`
    // realizes the split into the pool (a lone export has nothing to dominate).
    let rig = Rig::pg_batch(&format!("public.{table}"))
        .mode("chunked")
        .export_line("chunk_column: id")
        .export_line("chunk_size: 75000")
    ...
    for d in [rig.out_dir(), rig.out_dir_for("pr_grow_small")] {
        let _ = std::fs::remove_dir_all(&d);
        std::fs::create_dir_all(&d).unwrap();
    }

    // Run 1: split + crash a unit mid-way, leaving a partial split.
    let crashed = run_rivet_env(
        &["apply", cfg.to_str().unwrap(), "--pool", "2", "--split"],
        &[("RIVET_TEST_ERROR_AT", "chunk_export:1")],
    );
    ...
    // The source GROWS before the resume — the exact input that shifts re-sampled
    // boundaries and, pre-fix, drops a band of the original range on resume.
    c.batch_execute(&format!(
        "INSERT INTO {table} SELECT g AS id, md5(g::text) AS a \
    ...
    // Run 2: split + resume. Must reconstruct the ORIGINAL partition, not re-sample.
    let resumed = run_rivet_env(
        &[
            "apply",
            cfg.to_str().unwrap(),
    ...
    // The finding-2 guarantee: EVERY original id is declared by the manifest after
    // resume. RED before the fix — a re-sampled resume drops a band of 1..=N where
    // the shifted crashed-unit window no longer meets its neighbours' coverage.
    let declared = dir_manifest_copy_id_set(&rig.out_dir());
    let missing: Vec<i64> = (1..=N).filter(|id| !declared.contains(id)).collect();
    assert!(
        missing.is_empty(),
        "resume must preserve the ORIGINAL partition after source growth: {} of {N} \
         original ids are NOT declared by the manifest (e.g. {:?}) — a re-sampled resume \
         shifted the boundaries and dropped the crashed unit's original range",
        missing.len(),
