//! Crash-point recovery tests for the chunked export pipeline — MySQL twin
//! of `tests/live_chunked_recovery.rs`.
//!
...
//! are driver-agnostic. This file pairs each PG chunked-recovery test with a
//! MySQL twin so any regression of the chunked recovery semantics on the
//! MySQL driver is caught with the same coverage as on PG.
//!
//! ## Test symmetry with `live_chunked_recovery.rs`
//!
//! | PG test | MySQL twin (this file) |
//! |---|---|
//! | `chunked_crash_after_first_chunk_complete_resume_finishes_export` (C1)             | `mysql_chunked_crash_after_first_chunk_complete_resume_finishes_export`             |
//! | `chunked_crash_after_chunk_file_before_commit_resume_reruns_chunk_atleastonce` (C2) | `mysql_chunked_crash_after_chunk_file_before_commit_resume_reruns_chunk_atleastonce` |
//! | `parallel_chunked_crash_after_chunk_complete_resume_finishes_with_no_duplicates` (C3) | `mysql_parallel_chunked_crash_after_chunk_complete_resume_finishes_with_no_duplicates` |
//! | `parallel_chunked_crash_after_chunk_file_stuck_running_resume_reruns_chunk` (C4)    | `mysql_parallel_chunked_crash_after_chunk_file_stuck_running_resume_reruns_chunk`    |
//!
//! Test bodies are intentionally near-identical to the PG suite so any
//! deviation is easy to spot in review. The only differences are the source
//! driver, fixture seeding (MySQL `seed_mysql_numeric_table`), and the
//! YAML's `source.type`.
...
// ─── State-DB inspection helpers (identical to the PG twin) ──────────────────
...
fn mysql_chunked_crash_after_first_chunk_complete_resume_finishes_export() {
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_chunked_crash_after_chunk_file_before_commit_resume_reruns_chunk_atleastonce() {
    require_alive(LiveService::Mysql);
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_parallel_chunked_crash_after_chunk_complete_resume_finishes_with_no_duplicates() {
    // MySQL twin of `parallel_chunked_crash_after_chunk_complete_resume_finishes_with_no_duplicates`.
...
#[test]
#[ignore = "live: requires docker compose mysql"]
fn mysql_parallel_chunked_crash_after_chunk_file_stuck_running_resume_reruns_chunk() {
    require_alive(LiveService::Mysql);
