...
pub struct TuningConfig {
    ...
    /// Postgres: samples `pg_stat_bgwriter` checkpoint pressure. MySQL: samples
    /// `Created_tmp_disk_tables` + `Innodb_buffer_pool_wait_free` (read-spill
    /// pressure). SQL Server: `Workfiles Created/sec` + `Worktables Created/sec`
    /// (tempdb spills). Also arms the OPT-2 concurrency governor when
    /// `parallel > 1`.
